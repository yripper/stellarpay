 ...
 * the request with a raw `fetch`, and on a 402 response, checks the challenged amount
 * against configured spend limits before replaying the request through the matching
 * payment leg.
 *
 * Only the x402 leg is implemented in this task — an `"mpp"` challenge throws
 * `UnsupportedChallenge` (the mpp leg lands in Task 14, which also completes this function).
 */
export function createPayingFetch(config: PayingFetchConfig): typeof fetch {
  ...
  return async function payingFetch(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
    ...
    if (protocol === "mpp") {
      throw new UnsupportedChallenge("mpp leg lands in Task 14");
    }

    tracker.checkAndReserve(parseX402Amount(probe), url);

    try {
      emitter.emit({ type: "paying", protocol, url });
      const payFetch = buildX402Fetch(fetch, { secret: resolveSecret(config), network: config.network, rpcUrl: config.rpcUrl });
